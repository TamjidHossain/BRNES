...
class Env:
    def __init__(self, gridHeight, gridWidth, playMode, noTarget, noAgent, noObs, noFreeway):
        ...
        # self.posX1 = np.random.choice(self.sX)
        # self.posY1 = np.random.choice(self.sY)

        # self.posX2 = np.random.choice(self.sX)
        # self.posY2 = np.random.choice(self.sY)

        # self.aPosList = [[self.posX1, self.posY1], [self.posX2, self.posY2]]
    ...
    def reset(self, playMode, noTarget, noAgent, noObs, noFreeway, gridWidth, gridHeight, epochVal, CriteriaVal,countVal):
        ...
        # initialize agents' position
        with open("./fileOut/positions"+str(CriteriaVal)+"_BRNETp_G30A0", "rb") as Pp:   # Unpickling
